    ...
    try {
      ...
      if (Array.isArray(apiProps) && apiProps.length > 0) {
        const mapped: Property[] = apiProps.map((p: any) => ({
          ...
          district_name: p.district_name || p.location?.name || 'دمياط الجديدة',
          address_detail: p.address_detail || p.address || '',
          ...
          images: p.images?.map((img: any) => typeof img === 'string' ? img : img.image_url).filter(Boolean) || (p.image_url ? [p.image_url] : []),
          ...
          views: p.views || 0,
          amenities: p.amenities?.map((a: any) => typeof a === 'string' ? a : (a.name || a.slug)).filter(Boolean) || [],
          tags: p.tags?.map((tag: any) => typeof tag === 'string' ? tag : (tag.name || tag.slug)).filter(Boolean) || [],
