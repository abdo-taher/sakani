        ...
        if (liveProp && isMounted) {
          const mapped: Property = {
            ...
            images: Array.isArray(liveProp.images) && liveProp.images.length > 0
              ...
              : (liveProp.image_url ? [liveProp.image_url] : ['https://images.unsplash.com/photo-1600585154340-be6161a56a0c?auto=format&fit=crop&w=1200&q=80']),
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-8 space-y-6">
            ...
            <div className="bg-white rounded-3xl border border-slate-200/90 shadow-sm overflow-hidden space-y-0">
              ...
              {/* Thumbnails row (Photos + Video Thumbnail) */}
              {(images.length > 1 || Boolean(property.video_url || (property.videos && property.videos.length > 0))) && (
                <div className="flex gap-2 p-3 bg-slate-50 border-t border-slate-200/80 overflow-x-auto">
                  {/* Dedicated Video Thumbnail Button */}
                  {(property.video_url || (property.videos && property.videos.length > 0)) && (
                    <button
                      type="button"
                      onClick={() => setMediaMode('video')}
                    ...
                    >
                      <img 
                        src={property.video_thumbnail_url || images[0]} 
                        alt="فيديو المعاينة" 
                        className="w-full h-full object-cover opacity-80 group-hover/vid:opacity-100" 
                      />
                      <div className="absolute inset-0 bg-black/40 flex flex-col items-center justify-center gap-0.5">
                        <div className="w-6 h-6 rounded-full bg-[#8D6A28] text-white flex items-center justify-center shadow">
                          <Play className="w-3 h-3 fill-current ml-0.5" />
                        </div>
                        <span className="text-[9px] font-black text-white bg-black/60 px-1 rounded">فيديو</span>
                      </div>
                    </button>
                  )}
            ...
            <div className="bg-white rounded-3xl border border-slate-200/90 shadow-sm p-6 sm:p-8 space-y-6">
              ...
              {/* Video Player Section (Supports Multiple Videos & YouTube & MP4) */}
              {(property.video_url || (property.videos && property.videos.length > 0)) && (
                <div id="video-section" className="border-t border-slate-100 pt-6">
                  <PropertyMultiVideoPlayer
                    videos={property.videos}
                    videoUrl={property.video_url}
                    videoThumbnailUrl={property.video_thumbnail_url}
                ...
                <div id="rooms-section" className="border-t border-slate-100 pt-6 space-y-4">
                  ...
                  <div className="grid gap-4 sm:grid-cols-2">
                    {property.detailed_rooms.map((room) => {
                      const roomImg = room.imageUrl || room.images?.[0] || property.images[0];
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-start gap-3">
                            <div 
                              onClick={() => roomImg && setPreviewRoomImage(roomImg)}
