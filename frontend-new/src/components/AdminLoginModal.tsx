...
export const AdminLoginModal: React.FC<AdminLoginModalProps> = ({
  ...
  const [username, setUsername] = useState('SakaniAdmin2026');
  const [password, setPassword] = useState('Password@123@sakani');
  ...
  const handleQuickDemo = () => {
    setUsername('SakaniAdmin2026');
    setPassword('Password@123@sakani');
  };
    ...
    <div className="fixed inset-0 z-50 overflow-y-auto bg-black/75 backdrop-blur-sm flex justify-center items-center p-4" dir="rtl">
      ...
      >
        ...
        <div className="bg-[#0F172A] text-white p-6 text-center relative">
          ...
          <h3 className="text-xl font-black">لوحة تحكم إدارة سكني</h3>
          <p className="text-xs text-slate-400 mt-1">سجل الدخول لإدارة العقارات والطلبات والرسائل</p>
        </div>
        ...
        <div className="p-6 space-y-4">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="p-3 bg-amber-50/60 rounded-xl border border-amber-200/60 text-xs text-slate-600 flex items-center justify-between">
              <span className="font-medium">بيانات الدخول الافتراضية محددة مسبقاً للتجربة</span>
              <button
                type="button"
                onClick={handleQuickDemo}
                className="text-[11px] font-bold text-[#8D6A28] hover:underline"
              >
                تعبئة
              </button>
            </div>
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="w-full py-3.5 rounded-xl gold-gradient gold-gradient-hover text-white font-black text-sm shadow-md transition-all active:scale-95 disabled:opacity-50 flex items-center justify-center gap-2 cursor-pointer"
            >
