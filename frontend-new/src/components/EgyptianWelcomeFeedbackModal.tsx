...
export const EgyptianWelcomeFeedbackModal: React.FC = () => {
  ...
  const sendEgyptianGreetingNotification = () => {
    try {
      ...
      if (typeof window !== 'undefined') {
        ...
        window.dispatchEvent(new CustomEvent('sakani_push_notification', {
          detail: welcomePayload,
        }));
      }
    } catch (e) {}
  };
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center p-0 sm:p-4 bg-slate-950/75 backdrop-blur-md transition-all duration-300 animate-fadeIn" dir="rtl">
      {/* Click outside backdrop to dismiss */}
      <div className="fixed inset-0" onClick={() => handleDismiss(false)} />

      {/* Egyptian Themed Card Box */}
      <div 
        className="relative z-10 w-full sm:max-w-xl bg-white sm:rounded-3xl rounded-t-3xl shadow-2xl border border-amber-900/10 overflow-hidden flex flex-col max-h-[92vh] animate-slideUp sm:animate-scaleUp"
      ...
      >
        ...
        <div className="relative bg-gradient-to-r from-slate-950 via-[#1a2333] to-slate-950 px-6 py-6 text-white overflow-hidden shrink-0 border-b border-amber-500/20">
          ...
          <button 
            ...
            aria-label="إغلاق"
            className="absolute top-4 left-4 p-2 text-slate-400 hover:text-white rounded-full bg-white/10 hover:bg-white/20 transition-colors backdrop-blur-sm cursor-pointer"
          >
            <X className="w-5 h-5" />
          </button>
