...
export const PropertyDetailModal: React.FC<PropertyDetailModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-black/75 backdrop-blur-sm flex justify-center items-start sm:p-4 md:p-6" dir="rtl">
      
      {/* Modal Container */}
      <div 
        className="relative bg-white w-full max-w-4xl min-h-screen sm:min-h-0 sm:rounded-3xl shadow-2xl overflow-hidden animate-fade-in my-auto pb-24 sm:pb-8"
      ...
      >
            ...
            {images.map((img, idx) => (
              <button
                ...
                className={`relative w-20 h-14 rounded-xl overflow-hidden shrink-0 border-2 transition ${
                  idx === currentImageIndex ? 'border-[#8D6A28] scale-105 shadow-sm' : 'border-transparent opacity-60 hover:opacity-100'
                }`}
              >
        ...
        <div className="sticky bottom-0 left-0 right-0 z-20 bg-white/95 backdrop-blur-md border-t border-slate-200 p-4 sm:px-8 shadow-2xl flex items-center justify-between gap-3">
